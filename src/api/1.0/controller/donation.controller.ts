...
export class DonationController implements IController {
  ...
  constructor() {
    ...
    this.routes.push({
      handler: this.GetDonationById,
      method: "GET",
      path: `/${DONATIONS_PREFIX}/:donationId`,
    });

    this.routes.push({
      handler: this.SendMailToDonator,
      method: "POST",
      path: `/${DONATIONS_PREFIX}/send-mail`,
    });
  }
  ...
  public async PayDonation(req: Request, res: Response) {
    try {
      ...
      const { amount, email, mobile, userName, userId }: DonationInitialProps =
        req.body;
    ...
    try {
      ...
      if ((await axiosResponse.data.data.responseCode) === "SUCCESS") {
        ...
        return UnAuthorized(res, {
          message: `payment failed with ${await axiosResponse.data.data
            .responseCode}`,
