...
export class WebsiteController implements IController {
  ...
  public async NewUserContact(req: Request, res: Response) {
    try {
      ...
      return Ok(
        res,
        `${newUserContact.fullName} your message has been received`
      );
    } catch (err) {
  ...
  public async NewDonationRecords(req: Request, res: Response) {
    try {
      const { key_id, key_secret, minimumDonation }: IDonationConfigProps =
        req.body;
  ...
  public async DonationConfigs(req: Request, res: Response) {
    try {
      const donationConfigs = await DonationConfig.find().sort({
        createdAt: -1,
      });
  ...
  public async DonationConfigById(req: Request, res: Response) {
    try {
      ...
      const donationConfig = await DonationConfig.findById({
        _id: donationConfigId,
      });
